...
class GlobalLocalAttention(nn.Module):
    ...
    def pad(self, x, ps):
        ...
        # 如果需要 padding，使用 replicate 模式替代 reflect
    ...
    def forward(self, x):
        ...
        out = self.attn_x(F.pad(attn, pad=(0, 0, 0, 1), mode='replicate')) + \
              self.attn_y(F.pad(attn, pad=(0, 1, 0, 0), mode='replicate'))
...
class HF_UNetFormer(nn.Module):
    def __init__(self,
                 ...
                 window_size=8,
                 num_classes=6,
                 in_channels=3,             # [新增] 支持多通道输入
                 out_indices=(1, 2, 3, 4)   # [新增] 适配不同骨干网络的 stage 数量
                 ):
        super().__init__()

        # [修改] 传入 in_chans 和 out_indices。移除固定的 output_stride=32 (ConvNeXt 默认就是 32)
