...
class MFFModule(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv = nn.Sequential(
            ...
            nn.GroupNorm(
                1, self.reduced_channels
            ),  # Replaced BatchNorm with GroupNorm (G=1 -> LayerNorm-like) to avoid conflict with Per-Image Norm
...
class InputChannelDropout(nn.Module):
    ...
    def forward(self, x):
        ...
        k = torch.randint(
            self.min_drop, self.max_drop + 1, (1,), device=x.device
        ).item()
...
class M3LSNet(nn.Module):
    def __init__(self, input_channels=7, num_classes=2, img_size=128):
        ...
        self.dec_conv1 = nn.Conv2d(
            384 + 192 // 5, 192, kernel_size=1
        )  # Concat MFF output (reduced dim)
